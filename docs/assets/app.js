const state = {
  manifest: null,
  loadedKey: null,
  data: { matches: [], appearances: [], teams: [], players: [] },
  filters: {
    competition: null,
    season: null,
    team: 'all',
...
function readUrlParams() {
  ...
  state.sort.key = get('sort', 'minutes');
...
function syncUrl() {
  ...
  if (state.sort.key !== 'minutes') params.set('sort', state.sort.key);
...
function rebuildIndexes() {
  playerById.clear(); teamById.clear(); matchById.clear();
...
function renderSummary() {
  ...
  const totalMinutes = state.rows.reduce((sum, row) => sum + row.minutes, 0);
  ...
  els.minutesSummary.textContent = `${numberFmt.format(totalMinutes)} 分`;
  els.statusText.textContent = `${numberFmt.format(state.rows.length)}件を表示中`;
}

function renderTable() {
  if (!state.rows.length) {
    els.statsBody.innerHTML = `<tr><td colspan="9" class="empty-state">条件に一致する選手がいません。</td></tr>`;
    return;
  }
  els.statsBody.innerHTML = state.rows.map((row) => `
    <tr class="clickable" data-player-key="${escapeHtml(row.key)}">
      ...
      <td class="numeric">${numberFmt.format(row.bench_appearances)}</td>
      <td class="numeric">${numberFmt.format(row.unused_reserve)}</td>
    </tr>
  `).join('');
}
...
function renderPlayerPanel(playerKey) {
  ...
  els.panelStats.innerHTML = [
    ...
    `先発 ${numberFmt.format(row.starts)}`,
    `リザーブ登録 ${numberFmt.format(row.reserve_selections)}`,
    `途中出場 ${numberFmt.format(row.bench_appearances)}`,
...
function exportCsv() {
  ...
  const header = ['player_id','player_name','team_id','team_name','position','minutes','appearances','starts','reserve_selections','bench_appearances','unused_reserve','start_date','end_date'];
  const rows = state.rows.map((r) => [
    r.player_id, r.player_name, r.team_id, r.team_name, r.position_label, r.minutes, r.appearances, r.starts,
  ...
  a.download = `player-stats-${state.filters.competition}-${state.filters.season}-${state.filters.start}_${state.filters.end}.csv`;
  a.click();
